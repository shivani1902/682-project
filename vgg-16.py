...
def read_image_tags():
    alltags = set()
    image_data = {}
    tags_file = open("../HARRISON/tag_list.txt")
    tags = tags_file.readlines()
   # print tags
    image_file = open("../HARRISON/data_list.txt")
    lines = image_file.readlines()
    count = 0
    for line in lines:
        path = os.path.join("../HARRISON/"+line.strip())
        if os.path.exists(path):
            image_data[line] = tags[count].split()
            for i in tags[count].split():
                alltags.add(i)
        count+=1
       # print image_data, alltags
    return image_data, list(alltags)
